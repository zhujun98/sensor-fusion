...
struct Car
{
  ...
  {
    // render bottom of a car
    viewer->addCube(position.x - 0.5 * dimensions.x, position.x + 0.5 * dimensions.x,
                    position.y - 0.5 * dimensions.y, position.y + 0.5 * dimensions.y,
    ...
    // render top of a car
    auto top_name = name + "Top";

    viewer->addCube(position.x - 0.25 * dimensions.x, 0.25 * position.x + dimensions.x,
                    position.y - 0.5 * dimensions.y, position.y + 0.5 * dimensions.y,
                    position.z + dimensions.z * 2/3, position.z + dimensions.z,
                    color.r, color.g, color.b, top_name);
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION,
                                        pcl::visualization::PCL_VISUALIZER_REPRESENTATION_SURFACE, top_name);
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR,
                                        color.r, color.g, color.b, top_name);
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, 1.0, top_name);
  }
