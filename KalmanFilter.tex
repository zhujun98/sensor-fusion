...
In UKF, the state vector is expanded to
%
\begin{equation}
	\mathbf{x}^a_k = [\mathbf{x}^T_k, \mathbf{w}^T_k, \boldsymbol{\nu}^T_k]^T. 
\end{equation}
%
Equation \ref{eq:UK_start} - \ref{eq:UK_end} are applied to the augmented state vector to calculate the corresponding sigma matrix \(\mathbf{\Gamma}^a_k\).
